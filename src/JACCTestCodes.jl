...
function run_benchmarks(backend::KA.Backend)
    SIZE = 100000000
    alpha = 2.0
    benchmark_1d_axpy(backend, alpha, SIZE)
    SIZE = 100000000
    benchmark_1d_dot(backend, SIZE)
    SIZE = 10000
    benchmark_2d_axpy(backend, alpha, SIZE)
    SIZE = 10000
    benchmark_2d_dot(backend, SIZE)
    SIZE = 1000
    benchmark_lbm(backend, SIZE)
    benchmark_cg(backend, SIZE)
end
...
function benchmark_1d_axpy(backend::KA.Backend, alpha, SIZE)
    x = ones(SIZE)
    y = ones(SIZE)
    dx = adapt(backend, x)
    dy = adapt(backend, y)
...
function benchmark_1d_dot(backend::KA.Backend, SIZE)
    x = ones(SIZE)
    y = ones(SIZE)
    dx = adapt(backend, x)
    dy = adapt(backend, y)
...
function benchmark_2d_axpy(backend::KA.Backend, alpha, SIZE)
    x = ones(SIZE,SIZE)
    y = ones(SIZE,SIZE)
    dx = adapt(backend, x)
    dy = adapt(backend, y)
...
function benchmark_2d_dot(backend::KA.Backend, SIZE)
    x = ones(SIZE,SIZE)
    y = ones(SIZE,SIZE)
    dx = adapt(backend, x)
    dy = adapt(backend, y)
...
function benchmark_lbm(backend::KA.Backend, SIZE)
    SIZE = 1000
...
function benchmark_cg(backend::KA.Backend, SIZE)
    SIZE = 100000000
    a3 = ones(SIZE)
    a2 = ones(SIZE)
    a1 = ones(SIZE)
